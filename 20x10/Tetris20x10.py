...
class Tetris:
    ...
    def _get_12board_props(self, board):
        ...
        total_bumpiness, max_bumpiness = self._bumpiness(board)
        sum_height, max_height, min_height = self._height(board)
        h1, h2 , h3, h4 , h5 , h6, h7, h8 ,h9 , h10 = np.sum(board, axis = 0)
        return np.array([lines, holes, total_bumpiness, sum_height,h1, h2 , h3, h4 , h5 , h6, h7, h8 ,h9 , h10, self.current_piece, self.next_piece ])
    ...
    def glimpse(self):
        ...
        piece = [(x+8,y+2) for x,y in Tetris.TETROMINOS[self.next_piece][0]]
        for x,y in piece:
            new_board[x][y] = 4
    ...
    def render(self):
        ...
        img = np.array(img)
        img=cv2.resize(img,(self.testing *25, Tetris.BOARD_HEIGHT * 25),interpolation = cv2.INTER_AREA)
